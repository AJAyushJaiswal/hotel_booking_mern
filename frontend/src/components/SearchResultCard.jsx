        ...
        <div className="border border-gray-300 rounded-lg p-6 mb-6 flex" key={hotel._id}>
            <div className="mr-6 w-80 h-52 relative">
                <div className="w-full flex justify-between px-4 absolute top-24">
                    <button className={`p-3.5 bg-slate-500 rounded-3xl opacity-70 hover:opacity-85 active:opacity-100 ${imageNo === 0 ? 'invisible' : ''}`} onClick={handleImagePrevBtn}>
                    ...
                    <button className={`p-3.5 bg-slate-500 rounded-3xl opacity-70 hover:opacity-85 active:opacity-100 ${imageNo === hotel.images.length-1 ? 'invisible' : ''}`} onClick={handleImageNextBtn}>
